...
$(function(){
	...
	if("${number}">0){
		...
		var $plus = $("#plus");
		var $minus = $("#minus");
		$add.html("已加入购物车").attr("onclick","");
		$number.html("${number}");
		$plus.attr("onclick","plus();");
		$minus.attr("onclick","minus();");
	}
})
...
function add(){
	...
	$minus.attr("onclick","minus();");
	var url = "addCart.do";
	var param = {message:"add",id:"${goods.id}"};
	$.post(url,param);
}
...
function plus(){
	...
	var $number = $("#number");
	var old = $number.html();
	$number.html(parseInt(old)+1);
	var url = "addCart.do";
	var param = {message:"plus",id:"${goods.id}"};
	$.post(url,param);
}
...
function minus(){
	...
	if((parseInt(old)-1)<1){
		$number.html("0");
		$add.html("加入购物车").attr("onclick","add();");
		$plus.attr("onclick","");
		$minus.attr("onclick","");
		var param = {message:"delete",id:"${goods.id}"};
		$.post(url,param);
	}else{
		$number.html(parseInt(old)-1);
		var param = {message:"minus",id:"${goods.id}"};
		$.post(url,param);
	}
}
</script>
		...
		<div id="fh5co-about" data-section="about">
			...
			<div class="fh5co-2col fh5co-text">
				...
				<p class="text-center to-animate">
					...
					&nbsp;&nbsp;&nbsp;
					<a href="chooseAddress.do" class="btn btn-primary btn-outline">进行结算</a>
				</p>
			</div>
		</div>
		<!-- 评价信息栏 -->
